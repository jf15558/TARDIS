...
#' 
#' @param mask A RasterStack of geographic masks containing 1 (unmasked) or NA 
#' (masked) values, or a single RasterLayer of the same
...
link_mask <- function(x, mode = "lines", alg = "v", verbose = TRUE) {
  
  # check x is correctly supplied
  if(!exists("mask")) {
    stop("Supply mask as RasterStack")
  }
  if(!class(mask)[1] %in% c("RasterStack", "RasterLayer")) {
    stop("Supply x as a RasterStack or RasterLayer")
  }
  if(class(mask)[1] == "RasterStack") {mask <- stack(mask)}
  if(!isLonLat(mask)) {
  ...
  for(i in 1:length(bar)) {
    ...
    if(verbose) {
      cat(paste0("Resolving mask [", i, "/", length(bar), "]\r"))
      if(i == length(bar)) {cat("\n")}
    }
    ...
    if(maxValue(bar[[i]]) > 1) {
      ...
      if(alg == "v") {
        ...
        poly <- st_as_sf(as.polygons(rast(bar[[i]])))
        ...
        if(length(unique(extract(bar[[i]], st_coordinates(lin)[,-3]))) != maxValue(bar[[i]])) {
          stop("v algorithm failed to link all islands. Retry with k")
        }
        ...
        while(maxValue(bar[[i]]) > 1) {
          ...
          linked <- cbind(extract(bar[[i]], cellFromXY(bar[[i]], lin[,1:2])),
                          extract(bar[[i]], cellFromXY(bar[[i]], lin[,3:4])))
          foo3 <- cbind(1:maxValue(bar[[i]]), components(graph_from_edgelist(linked))$membership)
          
          # reclassify raster into the new, aggregated clumps
          bar[[i]] <- reclassify(bar[[i]], foo3)
        }
        ...
        touches <- unlist(do.call(rbind, lapply(lin, function(x) {extract(rast(bar[[i]]), vect(x), ID = F, fun = function(x) {length(na.omit(x))})})))
