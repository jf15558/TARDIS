...
#' 
#' @param geog A RasterStack of digital elevation models recording a changing
#' landscape forwards in time (i.e. the first layer is the oldest), or a
#' single RasterLayer for a single time slice. The Raster object must be in
#' geographic projection, i.e. longitude-latitude. with elevations in metres.
#' Negative values are permissible (e.g. bathymetry), but missing values are
#' not permissible and should be replaced (e.g. with the average of the
#' surrounding cells).
...
#' @param mask A RasterStack (or RasterLayer if geog is also a RasterLayer) of
#' the same resolution and extent as geog, which will be used to designate
#' non-traversable cells in the corresponding layers in geog. This must contain
#' only 1 (unmasked) or NA (masked) values.
...
#' reported. Useful when dealing with large rasters or a RasterStack with
#' many layers
...
create_tardis <- function(geog, times = NULL, glink = 8, tlink = 1, mask = NULL, mask.check = TRUE, alg = "v", rotations = NULL, verbose = TRUE) {
  ...
  # geog = (foo)
  # mask = (masks)
  # times = c(seq(from = 5.25, to = 0, by = -0.5), 0)
  # glink = 8
  # tlink = 1
  # rotations = NULL
  # mask.check = TRUE
  ...
  if(!exists("geog")) {
    stop("Supply geog as RasterStack or RasterLayer")
  }
  if(!class(geog)[1] %in% c("RasterStack", "RasterLayer")) {
    stop("Supply geog as a RasterStack or RasterLayer")
  }
  if(!isLonLat(geog)) {
  ...
  if(class(geog)[1] == "RasterLayer") {geog <- stack(geog)}
  ...
  if(nlayers(geog) > 1) {
    ...
    if(!is.numeric(times) | length(times) != nlayers(geog) + 1) {
    ...
    if(is.null(rotations)) {
      ...
      if(length(rotations) != (nlayers(geog) - 1)) {
        stop("Rotations should have nlayers(geog) - 1 elements")
      }
      if(any(unlist(lapply(rotations, max)) > ncells(geog[[1]]))) {
  ...
  if(is.null(mask)) {
    mask <- geog
    mask[] <- 1
    add_links <- lapply(1:nlayers(mask), function(x) {cbind(NA, NA)})
    
  } else {
    
    if(!class(mask)[1] %in% c("RasterStack", "RasterLayer")) {
      stop("If not NULL, supply mask as a RasterStack or RasterLayer")
    }
    if(any(!unique(mask[]) %in% c(1, NA))) {
      stop("Mask layers can only contain 1 or NA values")
    }
    if(class(mask)[1] == "RasterLayer") {mask <- stack(mask)}
    
    if(!all(dim(geog) == dim(mask)) | extent(geog) != extent(mask)) {
      stop("geog and mask must all have the same extent, resolution, and number of layers")
    }
    if(any(is.na(mask[]))) {
      
      bar <- lapply(as.list(mask), clump)
      if(any(unlist(lapply(bar, maxValue)) > 1) & mask.check == FALSE) {
  ...
  gdat <- c(rows = nrow(geog), cols = ncol(geog), hres = res(geog)[1], vres = res(geog)[2],
            xmin = extent(geog)[1], xmax = extent(geog)[2], ymin = extent(geog)[3], ymax = extent(geog)[4])
  
  # generate blank graph with full structure and uniform edge weights
  ed <- adjacent(geog, cells = 1:ncell(geog), directions = glink, sorted = TRUE)
  ...
  glinked <- list()
  for(i in 1:nlayers(geog)) {
    
    if(verbose) {
      cat(paste0("Linking geog [", i, "/", nlayers(geog), "]\r"))
      if(i == nlayers(geog)) {cat("\n")}
    }
