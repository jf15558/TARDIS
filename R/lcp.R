...
#' @import raster sf geosphere cppRouting
#' @export
...
lcp <- function(tardis, weights = NULL, origin, dest, verbose = TRUE) {
  ...
  dest$cell <- cellFromXY(raster(nrows = tardis$gdat[1], ncols = tardis$gdat[2], ext = extent(tardis$gdat[5:8])), st_coordinates(dest)) +
  ...
  for(i in 1:length(paths)) {
    ...
    # cells to lines
    path_xy <- xyFromCell(raster(nrows = tardis$gdat[1], ncols = tardis$gdat[2], ext = extent(tardis$gdat[5:8])), paths[[i]] %% prod(tardis$gdat[1:2]))
